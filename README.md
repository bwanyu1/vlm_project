...
フィールドの違い
STAIR-CaptionsGAIAキャプション場所annotations.captioncaptions（直接）キャプション数5つ5つ（同じ）画像image（埋め込み）image_src（URL）言語日本語英語サイズ82k28k（train）ジャンル日常写真衛星画像（地球観測）

コードへの影響
utils.pyのget_captions()は変更不要です。 フォールバック処理としてcaptionsフィールドも既に対応しています：
python# utils.py（現状のコード）
for key in ["captions", "caption", "text", "description"]:
    val = item.get(key)  # GAIAは "captions" で直接取れる ✅
dataset.pyの画像取得部分だけ修正が必要です。 GAIAは画像がimageフィールドではなくimage_src（URL）に入っています：
python# 現状: item.get("image") で取得
# GAIA: item.get("image_src") がURL → requestsで取得が必要
---
